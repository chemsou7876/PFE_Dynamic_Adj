...
class PriSTI(nn.Module):
    ...
    def calc_loss(
        ...
        # ════════════════════════════════════════════════════════
        # CHANGEMENT : Passer observed_data et observed_mask
        #              au diffmodel pour calculer A_dynamic
        #
        # AVANT :
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples, itp_info,
               observed_mask):
        """
        CHANGEMENT : Ajout de observed_mask en argument
        """
        B, K, L = observed_data.shape
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                #
                # AVANT :
                #   predicted = self.diffmodel(diff_input, side_info,
                #       torch.tensor([t]).to(self.device), itp_info, cond_mask)
                #
                # APRÈS : ajout de observed_data, observed_mask
                # ════════════════════════════════════════════════
                predicted = self.diffmodel(
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            # ════════════════════════════════════════════════
            # CHANGEMENT : Passer observed_mask à impute()
            #
            # AVANT :
            #   samples = self.impute(observed_data, cond_mask,
            #       side_info, n_samples, itp_info)
            #
            # APRÈS : ajout de observed_mask
            # ════════════════════════════════════════════════
            samples = self.impute(
                observed_data, cond_mask, side_info, n_samples, itp_info,
                observed_mask
            )
...
# ════════════════════════════════════════════════════════════
# Les sous-classes restent IDENTIQUES
# process_data() ne change pas
# ════════════════════════════════════════════════════════════
